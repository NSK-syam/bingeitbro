...
import { isNativeAppleSignInSupported } from '@/lib/native-webview';
...
const trustedSources = [
  {
    href: 'https://editorial.rottentomatoes.com/guide/best-movies-of-all-time/',
    label: 'Rotten Tomatoes',
    detail: 'Critics and audience-approved movie lists',
  },
  {
    href: 'https://www.boxofficemojo.com/chart/top_lifetime_gross/',
    label: 'Box Office Mojo',
    detail: 'All-time box office charts and release momentum',
  },
  {
    href: 'https://www.nielsen.com/news-center/2025/new-gracenote-report-highlights-impact-of-ineffective-content-discovery-on-consumer-happiness-with-streaming/',
    label: 'Nielsen Gracenote',
    detail: '2025 streaming discovery research and viewing behavior',
  },
] as const;
...
const discoveryStats = [
  {
    value: '12 min',
    summary: 'Average time U.S. viewers spend deciding what to watch.',
    href: 'https://www.nielsen.com/news-center/2025/new-gracenote-report-highlights-impact-of-ineffective-content-discovery-on-consumer-happiness-with-streaming/',
    source: 'Nielsen Gracenote, 2025',
  },
  {
    value: '19%',
    summary: 'Viewers who give up when they cannot find something to stream.',
    href: 'https://www.nielsen.com/news-center/2025/new-gracenote-report-highlights-impact-of-ineffective-content-discovery-on-consumer-happiness-with-streaming/',
    source: 'Nielsen Gracenote, 2025',
  },
] as const;
...
const landingStructuredDataJson = JSON.stringify([
  {
    ...
    description:
      'Friend-powered movie recommendations and TV picks to help you decide what to watch next faster.',
    inLanguage: 'en',
  },
  {
    '@context': 'https://schema.org',
        ...
        >
          ...
          <div className="relative">
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              ...
              <button
                type="button"
                onClick={() => onLockedActionClick(iosReviewMode ? 'schedule_watch' : 'share_with_friend')}
                className="rounded-full border border-white/15 bg-white/5 px-3 py-1.5 text-xs font-medium text-zinc-100 hover:bg-white/10"
              >
                {iosReviewMode ? 'Set reminder (locked)' : 'Share with friend (locked)'}
              </button>
            </div>
          </div>
        </article>
...
export default function HomeGate() {
  ...
  const previewTabs: PreviewTab[] = iosReviewMode ? ['movies', 'shows'] : ['movies', 'shows', 'friends'];
  const activePreviewTab: PreviewTab = iosReviewMode && previewTab === 'friends' ? 'movies' : previewTab;
  ...
  useEffect(() => {
    ...
    const syncAppleAvailability = () => {
      setShowAppleSignIn(isNativeAppleSignInSupported());
    };

    syncAppleAvailability();

    const intervalId = window.setInterval(syncAppleAvailability, 250);
    const stopPollingId = window.setTimeout(() => window.clearInterval(intervalId), 4000);
    window.addEventListener('bib-native-shell', syncAppleAvailability);

    return () => {
      window.clearInterval(intervalId);
      window.clearTimeout(stopPollingId);
      window.removeEventListener('bib-native-shell', syncAppleAvailability);
    };
  }, [mounted]);
  ...
  useEffect(() => {
    ...
    if (loading) return;
    if (!user) return;
    const hubToOpen = iosReviewMode && defaultHub === 'songs' ? 'movies' : (defaultHub ?? 'movies');
    if (!defaultHub || (iosReviewMode && defaultHub === 'songs')) safeLocalStorageSet('bib-default-hub', hubToOpen);
    router.replace(`/${hubToOpen}`);
  }, [mounted, loading, user, defaultHub, iosReviewMode, router]);
  ...
  const continueWithApple = async (source: string) => {
    setCtaError('');
    setAppleBusy(true);
    trackFunnelEvent('oauth_start', { source, location: 'landing', provider: 'apple' });
    const { error } = await signInWithApple();
    if (error) {
      setAppleBusy(false);
      setCtaError(error.message);
      trackFunnelEvent('oauth_error', { source, location: 'landing', provider: 'apple', message: error.message.slice(0, 120) });
      return;
    }
    trackFunnelEvent('oauth_redirect_started', { source, location: 'landing', provider: 'apple' });
    window.setTimeout(() => setAppleBusy(false), 4000);
  };

  const handlePreviewTab = (tab: PreviewTab) => {
    const nextTab = iosReviewMode && tab === 'friends' ? 'movies' : tab;
    setPreviewTab(nextTab);
    trackFunnelEvent('preview_tab_switch', { tab: nextTab });
  };

  const handleLockedAction = (action: string) => {
    trackFunnelEvent('preview_locked_action_click', { action, tab: activePreviewTab });
  ...
  if (!user) {
    return (
      <div className="min-h-screen bg-[var(--bg-primary)] relative overflow-hidden">
        ...
        <script type="application/ld+json">{landingStructuredDataJson}</script>
            ...
            <div className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
              <div className="space-y-5">
                <div>
                  ...
                  <h1 className="mt-3 text-4xl sm:text-5xl lg:text-6xl font-bold leading-tight text-white">
                    Best Movie Recommendations from Friends – Find What to Watch Next
                  </h1>
                  <p className="mt-4 max-w-2xl text-lg leading-8 text-zinc-300">
                    {iosReviewMode
                      ? 'BiB helps you turn trending discovery, watchlist saves, and reminder tools into faster movie recommendations without endless scrolling.'
                      : 'BiB helps you turn friend signals, trending titles, and watchlist tools into better movie recommendations and faster decisions on what to watch next.'}
                  </p>
                </div>

                <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                  <p className="text-xs uppercase tracking-[0.18em] text-amber-200">Trusted recommendation signals</p>
                  <div className="mt-3 grid gap-3 sm:grid-cols-3">
                    {trustedSources.map((source) => (
                      <div key={source.label} className="rounded-xl border border-white/10 bg-black/20 p-3">
                        ...
                        <p className="mt-2 text-sm leading-6 text-zinc-300">{source.detail}</p>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="mt-4 grid gap-3 sm:grid-cols-2">
                    {discoveryStats.map((stat) => (
                      <div key={stat.value} className="rounded-xl border border-amber-300/20 bg-amber-500/10 p-4">
                        <p className="text-2xl font-semibold text-white">{stat.value}</p>
                        <p className="mt-2 text-sm leading-6 text-zinc-200">{stat.summary}</p>
                        <p className="mt-2 text-xs text-zinc-400">
                          Source:{' '}
                          <cite className="not-italic">
                            <a href={stat.href} target="_blank" rel="noreferrer" className="hover:text-amber-200">
                              {stat.source}
                            </a>
                          </cite>
                        </p>
                  ...
                  <blockquote className="mt-4 rounded-2xl border border-white/10 bg-black/20 p-4 text-sm leading-7 text-zinc-100">
                  ...
                  <p className="mt-2 text-xs text-zinc-400">
                    Jared Grusd, CEO of Gracenote ·{' '}
                    <cite className="not-italic">
                      <a
                        href="https://www.nielsen.com/news-center/2026/google-gracenote-expand-global-collaboration-with-launch-of-ai-powered-tv-listings/"
                        target="_blank"
                        rel="noreferrer"
                        className="hover:text-amber-200"
                      >
                        Nielsen, February 2026
                      </a>
                    </cite>
                  </p>
                </div>
                ...
                <div className="grid gap-3 sm:grid-cols-3">
                  ...
                  <div className="rounded-xl border border-white/10 bg-white/5 p-3">
                    ...
                    <p className="mt-1 text-sm text-white">{iosReviewMode ? 'Save, sync, schedule' : 'Save, follow, groups, schedule'}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="rounded-2xl border border-white/10 bg-[rgba(8,8,12,0.9)] p-5 sm:p-6 shadow-[0_20px_60px_rgba(0,0,0,0.35)]">
                ...
                <p className="mt-2 text-sm leading-6 text-zinc-300">
                  {iosReviewMode
                    ? 'You can preview the interface below. Sign in only when you want to save picks, sync your account, or schedule watch reminders.'
                ...
                <div className="mt-5 grid gap-3">
                  {showAppleSignIn ? (
                    <button
                      type="button"
                      onClick={() => void continueWithApple('hero_apple')}
                      className="w-full rounded-2xl border border-white/15 bg-black px-4 py-3 text-sm font-semibold text-white transition hover:bg-zinc-900 disabled:opacity-60"
                      disabled={oauthBusy}
                    >
                ...
                <div className="mt-5 flex flex-wrap gap-2 text-xs text-zinc-300">
                  ...
                  {showAppleSignIn ? (
                    <span className="rounded-full border border-white/10 bg-white/5 px-3 py-1.5">Native Apple sign-in on iOS app</span>
                  ) : null}
          ...
          <section className="mt-7 rounded-3xl border border-white/10 bg-[rgba(10,10,14,0.76)] p-5 sm:p-6 lg:p-7 backdrop-blur-md">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <div>
                ...
                <h2 className="mt-2 text-2xl sm:text-3xl font-semibold text-white">
                  {iosReviewMode ? 'Browse discovery features before signing in' : 'Browse trending and friend features before signing in'}
                </h2>
                <p className="mt-2 text-sm text-zinc-300">
                  {iosReviewMode
                    ? 'You can look around here. Trying locked actions like save or schedule opens sign-in.'
            ...
            <div className="mt-5">
              <div className="mb-4 grid gap-3 lg:grid-cols-2">
                {!iosReviewMode ? (
                  ...
                  <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                    <p className="text-xs uppercase tracking-[0.16em] text-amber-300">Watchlist teaser</p>
                    <p className="mt-2 text-sm text-zinc-200">
                      Save promising movies and shows to your watchlist so they are ready when you need a pick.
                    </p>
                    <button
                      type="button"
                      onClick={() => handleLockedAction('watchlist_teaser')}
                      className="mt-3 rounded-xl border border-white/10 bg-black/25 px-3 py-2 text-sm text-zinc-100 hover:bg-white/5"
                    >
                      Save to watchlist (locked)
                    </button>
                ...
                <div className="rounded-2xl border border-white/10 bg-white/5 p-4">
                  ...
                  <p className="mt-2 text-sm text-zinc-200">
                    {iosReviewMode
                      ? 'Schedule a watch time and set reminders so good picks do not get buried.'
              ...
              {activePreviewTab === 'movies' ? (
                <PreviewCards cards={moviePreview} posterPaths={previewPosterPaths} onLockedActionClick={handleLockedAction} iosReviewMode={iosReviewMode} />
              ) : null}
              {activePreviewTab === 'shows' ? (
                <PreviewCards cards={showPreview} posterPaths={previewPosterPaths} onLockedActionClick={handleLockedAction} iosReviewMode={iosReviewMode} />
              ) : null}
              {!iosReviewMode && activePreviewTab === 'friends' ? (
            ...
            <div className="mt-5 rounded-2xl border border-white/10 bg-white/5 p-4">
              <p className="text-sm text-zinc-200">
                ...
                {' '}
                {iosReviewMode
                  ? 'titles shown here are a demo to show the product flow. Sign in to save picks, sync your account, and schedule watch reminders.'
