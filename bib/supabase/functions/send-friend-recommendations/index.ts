...
const rawServiceKey = Deno.env.get('SUPABASE_SERVICE_ROLE_KEY') ?? Deno.env.get('SERVICE_ROLE_KEY') ?? '';
...
/** Read body up to maxBytes to avoid OOM on large payloads. */ async function readBodyCapped(req, maxBytes) {
...
Deno.serve(async (req)=>{
  ...
  try {
    ...
    if (serviceKeyIsJwt) {
      const recipientIds = [
        ...new Set(toInsert.map((rec)=>rec.recipient_id))
      ];
      if (recipientIds.length > 0) {
        const friendsResponse = await fetch(`${supabaseUrl}/rest/v1/friends?select=friend_id&user_id=eq.${user.id}&friend_id=in.(${recipientIds.join(',')})`, {
          method: 'GET',
          headers: {
            apikey: supabaseAnonKey,
        ...
        if (friendsResponse.ok) {
          const friends = await friendsResponse.json().catch(()=>[]);
          const allowedIds = new Set(friends.map((row)=>row.friend_id).filter(Boolean));
    ...
    if (!insertResponse.ok) {
      ...
      const message = typeof data === 'object' && data !== null && 'message' in data ? String(data.message) : insertResponse.statusText;
      if (code === '23505') {
        return new Response(JSON.stringify({
          code: '23505',
          message: 'DUPLICATE'
