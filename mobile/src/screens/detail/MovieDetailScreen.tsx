...
export function MovieDetailScreen({
  route,
}: NativeStackScreenProps<DiscoverStackParamList, 'MovieDetail'>) {
  const { title, loading, error, refresh, providers } = useTitleDetail('movie', route.params.tmdbId);
  const watchlist = useWatchlist();
  const savedItem = useMemo(
    () =>
      watchlist.items.find(
        (item) => item.mediaType === 'movie' && item.tmdbId === route.params.tmdbId,
      ) ?? null,
  ...
  if (!title) {
    return (
      <ErrorState
        message="Movie details are unavailable right now."
        onRetry={refresh}
      ...
      >
        ...
        <View style={styles.watchlistCard}>
          <View style={styles.watchlistCopy}>
            <Text style={styles.watchlistTitle}>Save this movie</Text>
            <Text style={styles.watchlistBody}>
              Add the title to your watchlist so it is waiting in the Watchlist tab.
            </Text>
          </View>
          <Pressable
            accessibilityLabel={savedItem ? 'Remove movie from watchlist' : 'Add movie to watchlist'}
